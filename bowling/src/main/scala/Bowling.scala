class Bowling(points: Int, previousScore: Int, rollTurn: Int, nextHasToBeTwice: Boolean):
  ...
  def roll(score: Int): Bowling =
    if rollTurn % 2 == 0 then
      if score + previousScore == 10 then return new Bowling(points + score, score, rollTurn + 1, true)
    if rollTurn % 2 == 1 && nextHasToBeTwice then return new Bowling(points + score*2, score, rollTurn + 1, false)
